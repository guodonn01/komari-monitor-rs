...
pub async fn dry_run() {
    ...
    info!("Memory: {} MB / {} MB", mem.used / 1000 / 1000, mem_with_out_usage.mem / 1000 / 1000);
    info!("Swap: {} MB / {} MB", swap.used / 1000 / 1000, mem_with_out_usage.swap / 1000 / 1000);

    let load = realtime_load();
    info!("Load: {:.2} / {:.2} / {:.2}", load.load1, load.load5, load.load15);
    ...
    for disk in disks {
        info!("{} | {} | {} | {} GB / {} GB", disk.name().to_string_lossy(), disk.file_system().to_string_lossy(), disk.mount_point().to_string_lossy(), disk.available_space() / 1000 / 1000/ 1000, disk.total_space() / 1000 / 1000/ 1000);
    }
    ...
    for (name, data) in networks.iter() {
        ...
        if should_filter || data.mac_address().0 == [0, 0, 0, 0, 0, 0] {
            ...
            info!("{} | {} | UP: {} GB / DOWN: {} GB", name, data.mac_address().to_string(), data.total_transmitted() / 1000 / 1000 / 1000, data.total_received() / 1000 / 1000 / 1000)
        }
    }
